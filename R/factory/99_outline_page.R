...
sec('Where can play-calling obviously be improved?',
    note('Michael asked where the obvious improvements are. The answer is not that the early-down run is a mistake, which everyone has said. It is the <b>scale gap</b>. Fourth down is the leak the internet argues about and it comes up about <b>half a time per team per game</b>. Running on early downs in spots where passing clearly wins comes up <b>16 times a game</b>. Thirty times the volume, a fraction of the attention.'),
...
sec('What each man actually calls',
    note('From the SumerSports charting: 216,857 plays, 193 fields, mined for coach-level traits and then adversarially re-checked. Twenty-seven measures survived. These two grids are the useful shape of it. Read a coach across to see who he is, or a column down to see who is extreme at one thing. The number in each cell is his own rate; the colour is where that sits in the league.'),
    ...
    fig(file.path(FIG,'fingerprints_offense.png'),
        'Grid of offensive play-callers against eight scheme measures, coloured by league percentile',
    ...
    fig(file.path(FIG,'fingerprints_defense.png'),
        'Grid of defensive coordinators against eight scheme measures, coloured by league percentile',
...
sec('The play everyone agrees works, that nobody runs',
    fig(file.path(FIG,'sneak.png'),
        ...
        'The sneak is worth +0.35 EPA against +0.02 for any other run inside two yards, converts 81% against 67%, and is better on every single down. It is used on 13% of short-yardage runs. Philadelphia sneaks on 37% of them and Kansas City on 1%. This one is a franchise habit rather than a coach one: the team version repeats at r = +0.77 while the caller version does not survive scrutiny.')),
