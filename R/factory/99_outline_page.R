...
sec('McVay (offense)',
    ...
    note('For the movement section: nobody&rsquo;s data counts motions (checked Sumer&rsquo;s full schema; FTN flags presence only), so the closest honest quantification is the variety of pre-snap <b>pictures</b> a caller shows, built from every skill player&rsquo;s alignment across 4.8 million player-plays. McVay&rsquo;s answer is the surprise that organizes the whole segment: <b>32nd of 36</b> in picture variety, 55% bunched sets against a league at 39%, and the <b>leakiest pictures in football</b> (1st of 36 at revealing run vs pass beyond the situation). Yet his motion is the one in the league that genuinely scrambles a pre-snap model (6th of 37, interval clear of zero). The formula: a few recurring, individually leaky shapes, with players moving inside them until the leak is bought back. The movement is not many pictures; it is life inside a few. Shanahan is the mirror: 3rd of 36 in variety, mid-pack leak, below-median motion effect, deception by menu instead.'),
    fig(file.path(OLD,'presnap_lie.png'),
        'Picture variety against picture leak by caller, McVay isolated in the few-and-leaky corner, Shanahan high-variety',
    ...
    fig(file.path(OLD,'presnap_variety.png'),
        'Pre-snap picture variety leaderboard with the three names marked',
        'How many different pictures each caller shows. Built by R/25.')),
