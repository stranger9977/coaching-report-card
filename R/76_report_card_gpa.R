...
# and tiers, for every head coach graded on the WAR board.
#
...
w  <- fread("data/derived/coaching_war.csv")[eligible == TRUE & !is.na(rank)]
...
cat(sprintf("%d coaches on the card, %d with a GPA (4+ lines)\n", nrow(rc), sum(!is.na(rc$gpa))))
cat("tiers:\n"); print(rc[, .N, by = tier])
cat("\nthe card:\n")
print(rc[, c("gpa_rank", "coach", "seasons", gcols, "gpa", "tier"), with = FALSE][, lapply(.SD, function(x) if (is.numeric(x)) round(x, 2) else x)], nrows = 60)
...
out <- rc[, c("gpa_rank", "coach", "seasons", "gpa", "tier", "n_lines", gcols, names(LINES), "r_mkt", "r_qb", "r_pd"), with = FALSE]
...
p <- ggplot(g, aes(x = gpa, y = reorder(coach, gpa), colour = tier)) +
  ...
  labs(title = sprintf("The report card: %s on the dean's list, %d on probation; the GPA and WAR agree on the order (rank r = %.2f) but not the grades",
                       if (sum(g$tier == "Dean's list") == 1) paste("only", g[tier == "Dean's list"]$coach) else paste(sum(g$tier == "Dean's list"), "coaches"),
                       sum(g$tier == "Probation"), cor(g$gpa, g$war, method = "spearman")),
       subtitle = paste0("GPA out of 4.0 over seven lines, each a letter grade by percentile among these coaches: fourth-down decisions, going for two when the chart\n",
                         "says to, penalties, offense and defense above talent, beating the spread, and results above resources (the three WAR boards). Unweighted.\n",
                         "Letters in the label are in that order; a dash is an incomplete. Dotted lines are the tier cuts: 2.0 passing, 2.7 honor roll, 3.3 dean's list."),
       x = "GPA", y = NULL,
       caption = fig_caption("nflverse, SumerSports and OverTheCap via the scripts named in R/76's header, 2012-2025 (fourth downs 2018-2025)",
         "\nA GPA needs at least four of the seven lines. Built by R/76.")) +
