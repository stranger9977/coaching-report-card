...
sec('McVay (offense)',
    ...
    note('Players and coordinators are quoted saying he "makes 25 plays seem like 250." The four measurements above now live in one composite for the camera: 5th-fewest pictures, personnel held 84% (1st), 4th-most play variety inside the held stretches, and the biggest motion effect on run/pass guessability that is clearly real rather than noise. The twist to say out loud: his static pictures tip the play MORE than anyone&rsquo;s; the disguise is what moves inside them, not the picture count.'),
    fig(file.path(OLD,'mcvay_formula_diagram.png'),
        'Flow diagram of the McVay method: same players, same picture, late motion, any play in the menu',
        'The mechanism, drawn. Four single-claim charts (mcvay_formula_pictures/hold/variety/motion.png) carry the numbers behind each box. Built by R/42.'),
    note('The "tips run vs pass" axis, explained on his own looks (the asked-for depth): out of 11 personnel, 3x2, shotgun he throws it <b>96%</b> when the situation said 67%; every shotgun look throws more than expected (+6 to +29 points), every under-center look runs more (-4 to -19). <b>Most of the tell is the quarterback&rsquo;s feet.</b> No model: league pass rate per down-and-distance bucket against his own calls.'),
    fig(file.path(OLD,'tip_explainer.png'),
        'Ten McVay looks, expected pass rate against actual, shotgun looks all passing more and under-center looks all running more',
        'The tip, made concrete. Built by R/43.'),
    note('And the quote&rsquo;s direct test, menu against wardrobe: McVay has the <b>3rd-smallest play menu</b> of 36 (47 plays cover 80% of snaps, league 56) but ranks <b>27th in dressings per play</b>: few plays, worn the same way. The static-wardrobe half of the folklore actually belongs to <b>Ben Johnson</b> (5th-smallest menu, 6th-biggest wardrobe). McVay&rsquo;s 250 lives in the motion and the within-look play menu, not the outfit rack. Both axes are stable traits (r = +0.81 / +0.72).'),
    fig(file.path(OLD,'menu_wardrobe.png'),
        'Scatter of play-menu size against dressings per play with McVay few-plays-same-dressing and Johnson in the folklore corner',
        'Menu against wardrobe, all callers cut to the same sample. Built by R/41.'),
    note('The single-game version, film-ready and from this past season: vs New Orleans, week 9 of 2025-26, he showed the same picture 20 times (13 personnel, 2x2, under center) and called <b>11 different plays</b> out of it, three of them touchdowns; every snap is tabled with quarter and clock for clipping. The biggest example if an older game is fine: at Seattle, week 13 of 2022-23, 28 snaps, 15 plays. He motions on <b>62% of snaps</b> (4th of 37, league median 46%), so the motion-effect comparison is his own other 38%.'),
    fig(file.path(OLD,'one_look_one_game.png'),
        'Film-card table of 20 snaps of one McVay look against New Orleans this past season, 11 distinct plays with quarter, clock and result',
        'One game, one look, eleven plays. Built by R/44.'),
    note('And the direct test of "hard to predict," after pushback that run/pass is too reductive: <b>guess the actual play</b> (concept and side, or routes and depth), knowing the look and the situation, never using the snap being guessed, everyone on the same 1,200 plays. <b>McVay comes out 33rd of 36 from hardest: one of the easiest callers in football to guess</b>, confirmed with a second independent play definition. The film feels unpredictable because of what surrounds the play, not because the play is hard to name. The full arc in one sentence: third-smallest menu, most-held personnel, leakiest looks, guessable plays, elite anyway; being hard to guess is not what makes him good. The deception stories live elsewhere: for <b>Shanahan a defense guesses the play better BEFORE he lines up than after</b> (3rd-biggest backfire in football; Johnson&rsquo;s look backfires too), and the mechanism is menu volume: each of his many looks is too rare to learn from.'),
    fig(file.path(OLD,'guess_the_play.png'),
        'Callers ranked by how often the defense&rsquo;s best pre-snap play guess is right, McVay near the easiest end, Shanahan and Johnson with looks that mislead',
        'The play-level guessing game. Built by R/46.'),
    note('His pistol experiment, from the alignment charting: none charted in 2022-23, then 7.7% of snaps in 2023-24 (league 4.3%) and 10.6% in 2024-25 (league 5.8%), roughly double the league both years, then abandoned this past season (0.7%) in the same offseason as the 13-personnel pivot. Against Seattle specifically it barely appeared (0/4/2/0 snaps across the four meetings).')),
...
sec('Defense: showing one coverage and playing another',
    ...
    note('Michael: <i>"I wonder if post snap movement out of shell has increased over time&hellip;my guess is yes."</i> Right: 29.1% of dropbacks in 2022 to 32.3% in 2025, rising 1.2 points a season (a climb too steady to be noise), and the charting&rsquo;s missing-shell rate is under 1.1% and stable, so it is not a labeling artifact.'),
    ...
    note('Asked directly whether the EPA stuff on safety rotation was double-checked: it is now QUADRUPLE-checked. The fourth cut, coordinator by coordinator (in case rotation only pays for the men who are good at it): <b>43 coordinators tested, 2 clear the 95% bar, and 2.1 would clear it by luck alone</b>, one in each direction. Macdonald leans the right way (-0.09) with an interval spanning zero. Nobody&rsquo;s rotation provably pays.'),
    fig(file.path(OLD,'rotation_by_dc.png'),
        'Forty-three coordinators by situation-controlled EPA gap between rotated and static snaps, every interval crossing zero except two at the chance rate',
        'The fourth null: rotation payoff by coordinator. Built by R/45.'),
